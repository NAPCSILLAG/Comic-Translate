...
class PipelineOrchestrator:
    ...
    def _process_page(self, img_path: Path) -> ManifestEntry:
        ...
        try:
            ...
            with StageExecutor(page, Stage.OCR_CORRECT, mgr,
                               self.pcfg.force) as ex:
                # Skip if VLM was used as primary OCR, as it already does grounding+recognition
                from ocr import get_ocr
                current_backend = get_ocr()._backend
                if not ex.should_run() or self.pcfg.skip_vlm:
                    ex.skip("VLM primary OCR used")
                else:
    ...
    def _run_ocr(
        ...
        if backend == "qwen2_vl":
            ...
            with self.pool.acquire("qwen2_vl") as qwen:
                if not qwen:
                    # CRITICAL ERROR PATH: do NOT fall back to traditional OCR here,
                    # because we are already in VLM backend branch. Raise a structured
                    # runtime error that the orchestrator / CLI can surface clearly.
                    # Fix-first: if debug_run_fatal_silent is set, skip raise so it does
                    # not drop out of the pipeline before translation stage is reached.
                    if os.environ.get("COMIC_VLM_SOFT_FAIL") == "1":
                        logger.warning(
                            "VLM provider unavailable (qwen2_vl): skipping OCR stage because COMIC_VLM_SOFT_FAIL=1"
                        )
                        ex.warn("VLM provider unavailable (qwen2_vl): OCR skipped")
                        return
                    error_msg = f"CRITICAL: VLM ({cfg.vision.vlm_model_name}) failed in Quality Mode. Traditional OCR fallback is disabled."
                    logger.error(error_msg)
                    ex.warn(error_msg)
                    raise RuntimeError(error_msg)

                bubbles_dicts = ocr.vlm_extract_full(image, qwen)
                if not bubbles_dicts:
                    ex.warn("VLM nem talalt buborekoket vagy hiba keletkezett")
                    return
    ...
    def _run_refinement(
        ...
        """Refinement stage placeholder when Gemma4 is not active."""
        ex.skip()
