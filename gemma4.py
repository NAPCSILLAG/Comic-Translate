...
class Gemma4Provider(LightweightProvider):
    ...
    def __init__(
        ...
        # Config-driven: nincs hardcode modell név
        self.model    = model or cfg.vision.gemma_model
        self.base_url = (
            base_url
            or cfg.vision.gemma_base_url
            or cfg.translation.base_url
        )
    ...
    def _check_model(self) -> None:
        """Ollama elérhetőség + modell ellenőrzés. Nem crash ha offline."""
        try:
            ...
            if resp.status_code == 200:
                ...
                if any(mbase in m for m in models):
                    logger.info(f"[gemma4] Modell elérhető: {self.model}")
                else:
                    logger.warning(
                        f"[gemma4] Modell nem talalhato: {self.model} "
                        f"| Letoltes: ollama pull {self.model} "
                        "| Config override: COMIC_GEMMA_MODEL env var")
            else:
                logger.warning(
                    f"[gemma4] Ollama nem válaszol (HTTP {resp.status_code})")
        except requests.exceptions.ConnectionError:
            logger.warning(
                f"[gemma4] Ollama nem elérhető: {self.base_url} – provider disabled")
        except Exception as e:
            logger.warning(f"[gemma4] Ellenőrzés hiba: {e}")
