...
def norm_to_pixel(norm_box: list[float], width: int, height: int) -> tuple[int, int, int, int]:
    """
...
class PPOCRv5Detector:
    ...
    def _unclip_polygon(
        ...
        try:
            ...
            # DB unclip távolság: area * ratio / perimeter
            area = w * h
            perimeter = 2 * (w + h)
            distance = area * ratio / perimeter
            
            # Kiterjesztés (unclip) a középpontból
            expanded_rect = (rect[0], (w + distance * 2, h + distance * 2), rect[2])
            box = cv2.boxPoints(expanded_rect)
            box = box.astype(np.float32)
            
            # Szigorú TL, TR, BR, BL sorrend kényszerítése
            rect_ordered = np.zeros((4, 2), dtype=np.float32)
            s = box.sum(axis=1)
            rect_ordered[0] = box[np.argmin(s)]   # Top-Left (x+y min)
            rect_ordered[2] = box[np.argmax(s)]   # Bottom-Right (x+y max)
            diff = np.diff(box, axis=1)
            rect_ordered[1] = box[np.argmin(diff)] # Top-Right (y-x min)
            rect_ordered[3] = box[np.argmax(diff)] # Bottom-Left (y-x max)
...
class PPOCRv5Recognizer:
    ...
    def _preprocess_crop(self, crop: np.ndarray, bubble_id: int = -1) -> np.ndarray:
        ...
        target_w = max(int(w * target_h / max(h, 1)), 32)  # minimum 32px szélesség (különben Conv.33 crash {1,0})
    ...
    def run(
        ...
        try:
            raw = self._reader.readtext(
                ...
                min_size=4,           # 5→4: kisebb szövegrészek is felismerhetők
                text_threshold=0.4,   # 0.6→0.4: lazább küszöb, több találat
                low_text=0.25,        # 0.3→0.25: alacsonyabb töredék-küszöb
                link_threshold=0.3,   # 0.4→0.3: szomszédos régiók összekapcsolása
            )
        except Exception as e:
...
class ComicOCR:
    ...
    def __init__(self) -> None:
        ...
        self._fallback: Optional[EasyOCRFallback] = None
        self._backend:  str                       = cfg.ocr.backend
        # Session-szintű PPOCRv5 feladás-számláló:
        # Ha ennyi egymást követő buborékon sikertelen a PPOCRv5,
        # az oldal végéig EasyOCR-t használunk (~50-60s/oldal megtakarítás).
        self._ppocr_consecutive_failures: int  = 0
        self._PPOCR_GIVE_UP_THRESHOLD:    int  = 3
        self._ppocr_session_disabled:     bool = False
        self._init_primary()
        self._init_fallback()
    ...
    def _init_primary(self) -> None:
        ...
        if self._backend in ("easyocr", "qwen2_vl"):
            if self._backend == "qwen2_vl":
                logger.info("OCR backend: Qwen2-VL (VLM grounding) ✓")
            else:
    ...
    def reset_session(self) -> None:
        """Session-számláló nullázás oldal-váltáskor (orchestrator hívja)."""
        self._ppocr_consecutive_failures = 0
        self._ppocr_session_disabled     = False
    ...
    def extract(
        ...
        if backend_mode == "qwen2_vl":
            # Quality Mode: Traditional OCR fallbacks are strictly disabled.
            # Full-image extraction is handled by vlm_extract_full() in the orchestrator.
            return []

        results: list[OCRResult] = []

        # Session-szintű PPOCRv5 feladás:
        # Ha már elegendő egymást követő hiba volt, közvetlen EasyOCR-t használunk.
        use_ppocr = (
            self._primary is not None
            and not self._ppocr_session_disabled
        )

        if use_ppocr:
            try:
                results = self._primary.run(
                    image_rgb, bbox=bbox, language=language,
                    bubble_idx=getattr(self, '_cur_bubble_idx', 0))
            except Exception as e:
                logger.warning(f"PPOCRv5 kivétel: {e}")
                results = []
        elif self._ppocr_session_disabled:
            logger.debug("OCR AUTO | PPOCRv5 session disabled → EasyOCR direct")
        ...
        if use_ppocr:
            logger.debug(
                f"OCR AUTO | backend=ppocr | valid={valid_count} | "
                f"avg_conf={avg_conf:.2f} | usable={usable} | reason={reason}"
            )

        if not usable:
            # PPOCRv5 egymást követő hiba-számláló frissítése
            if use_ppocr:
                self._ppocr_consecutive_failures += 1
                if (self._ppocr_consecutive_failures >= self._PPOCR_GIVE_UP_THRESHOLD
                        and self._fallback.available):
                    logger.info(
                        f"PPOCRv5 {self._ppocr_consecutive_failures}× egymás után "
                        f"sikertelen → session-szintű EasyOCR (gyorsabb feldolgozás)"
                    )
                    self._ppocr_session_disabled = True
            ...
            # PPOCRv5 sikeres → számláló nullázás
            if use_ppocr:
                self._ppocr_consecutive_failures = 0
    ...
    def vlm_extract_full(
        self,
        image_bgr: np.ndarray,
        provider: Any,
    ) -> list[dict]:
        """
        VLM-alapú teljes kinyerés (Grounding + OCR).
        Szakítja a Layout -> OCR ciklust: egyben adja vissza a buborékokat.
        """
        # BGR -> RGB
        image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        h, w = image_rgb.shape[:2]
        ...
        try:
            # A provider-t az orchestrator adja át a ProviderPool-ból
            result = provider.extract_text(image_rgb)
            if not result.success:
                logger.error(f"VLM extraction failed: {result.error}")
                return []

            data = result.data # Expecting {"bubbles": [...]}
            if not data or not isinstance(data, dict):
                logger.error(f"VLM returned invalid data format: {type(data)}")
                return []

            bubbles_data = data.get("bubbles", [])

            enriched_bubbles: list[dict] = []
            for i, b_raw in enumerate(bubbles_data):
                box_norm = b_raw.get("box_2d")
                text = b_raw.get("text", "")

                if not box_norm or len(box_norm) != 4:
                    continue

                # Koordináták konverzió: norm [ymin, xmin, ymax, xmax] -> pixel [x1, y1, x2, y2]
                pixel_box = norm_to_pixel(box_norm, w, h)

                # BubbleData kompatibilis struktúra
                bubble = {
                    "order": i,
                    "bbox": pixel_box,
                    "polygon": [
                    ...
                    "raw_text": text,
                    "ocr_confidence": 0.9, # VLM fixed confidence
                    "ocr_results": [
                        OCRResult(
                            text=text,
                            confidence=0.9,
                            polygon=tuple(
                                [(pixel_box[0], pixel_box[1]), (pixel_box[2], pixel_box[1]),
                                 (pixel_box[2], pixel_box[3]), (pixel_box[0], pixel_box[3])]
                            ),
                            bbox=pixel_box,
                        ).to_dict()
                    ]
                }
                enriched_bubbles.append(bubble)

            logger.info(f"VLM OCR kész: {len(enriched_bubbles)} buborék kinyerve")
            return enriched_bubbles

        except Exception as e:
            logger.exception(f"VLM full extraction critical crash: {e}")
            return []
    def process_page(
        self,
        image_bgr: np.ndarray,
